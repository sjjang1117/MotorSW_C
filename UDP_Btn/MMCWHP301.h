...
*     2011.05.02 : 1.55 : dll 종료시 homming thread 종료 루틴 변경 -> app 비정상 종료시 mmcwhp.dll 정상 종료되지 않음 건 개선
...
/// new homing define
//////////////////////
#define HOMING_DIR_NEGATIVE -1
#define HOMING_DIR_POSITIVE 1

#define HOMING_MODE_LIMIT_HOME_INDEX	1
#define HOMING_MODE_LIMIT_HOME			2
#define HOMING_MODE_LIMIT_INDEX			3
#define HOMING_MODE_LIMIT				4
#define HOMING_MODE_HOME				5
#define HOMING_MODE_INDEX				6
#define HOMING_MODE_CURRENT_POSITION	7
...
// JJS 2014-01-07
INT API set_joystick_enable(INT bdNum, INT Enable, pINT Joystick, pINT Axes);
INT API set_joystick_velocity(INT bdNum, INT vel0, INT vel1, INT vel2, INT vel3);
INT API get_joystick_enable(INT bdNum, pINT Enable, pINT Joystick, pINT Axes);
...
INT API mmc_homing_start(INT ax);
INT API mmc_homing_stop(INT ax);
INT API mmc_set_homing_parameter(INT ax, 
						ULONG Timeout, 
						double HomeOffset,
						INT Direction,
						INT Mode,
...
INT API mmc_get_homing_state(INT ax,pINT pState);
INT API mmc_get_homing_error(INT ax, pINT pError);
